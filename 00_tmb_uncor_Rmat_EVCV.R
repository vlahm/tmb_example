  ...
  for(int i=0;i<timeSteps;i++){ //move one time step at a time
	 ...
	 if(nonNAcount<obs.row(i).size()){ //if NA values present
		...
		ans += VECSCALE(MVNORM(subCorr),subSds)(subDiffer);
	   	...
	   	vector<Type> differ = obs.row(i)-pred.transpose().row(i);
	 	ans += VECSCALE(MVNORM(FullCorrMat),sdObs)(differ);
	 }//end of data likelihood for this time step
  }//end of loop over time steps
...
if(file.exists('dfa1tmb_EVCV.cpp')){
    t1<-read.table('dfa1tmb_EVCV.cpp',sep="\t",stringsAsFactors=F,comment.char='', quote = "")
...
runDFA<-function(obs,NumStates=1,ErrStruc='DE',EstCovar=FALSE,Covars=NULL,indivCovar=FALSE,Dmat=NULL,Dfac=NULL,Rfac=NULL,logsdObs=NULL,logsdObsFac=NULL,cholCorr=NULL,cholFac=NULL,EstSE=FALSE){
	...
	#
	#NumStates=1;ErrStruc='UEVCV';EstCovar=F;Covars=NULL;indivCovar=FALSE;Dmat=NULL;Dfac=NULL;Rfac=NULL;logsdObs=NULL;logsdObsFac=NULL;cholCorr=NULL;cholFac=NULL;EstSE=F
	#obs<-Z_L_FW1
	#Dmat<-DmatRun
	#Dfac<-DfacRun
	...
	if(ErrStruc=='DE'){
		...
		#return(NULL)
		CorrMat <- matrix(0,ncol(obs),ncol(obs))
		diag(CorrMat)<-1
		CorrMatFac<-matrix(1,ncol(obs),ncol(obs))
		diag(CorrMatFac)<-NA
		CorrMatFac<-factor(CorrMatFac)
		logsdObs<-log(rep(0.5,ncol(obs)))
		logsdObsFac<-rep(1,ncol(obs))
		logsdObsFac<-factor(logsdObsFac)
	...
	if(!(ErrStruc %in% c('EVCV','UEVCV'))){
		...
		covStateFac<-factor(matrix(NA,nrow=NumStates,ncol=NumStates))
		#Creates the model object and runs the optimization
		obj1 <- MakeADFun(data,parameters,random="u",DLL="dfa1tmb_EVCV",silent=T,map=list(Z=Zfac,D=Dfac,FullCorrMat=CorrMatFac,logsdObs=logsdObsFac,covState=covStateFac))#,map=list())
	}
